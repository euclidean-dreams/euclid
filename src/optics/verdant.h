...
public:
    ...
    void draw(Canvas &canvas) {
        if (luon.energy > energy_requirement) {
            ...
            // trough detected
            if (luon.prior_delta <= 0 && luon.delta > 0) {
                branches.emplace_back(luon, *(--body.end()), cos(rand() % 64 / 16) * M_PI / 3, luon.energy);
            }

            auto cur_branch = branches.begin();
            while (cur_branch != branches.end()) {
                if (not cur_branch->keep_alive())
            ...
            branches.clear();
            body.clear();
            origin = {scast<float>(rand() % canvas.width), scast<float>(rand() % canvas.height)};
...
public:
    Verdant(int width, int height, sp<Psyche> psyche)
        ...
        for (auto &luon: *psyche->luons) {
            ...
            stalks.emplace_back(luon, origin, scast<float>(2 * M_PI * cos(luon.fundamental / 64)), 3);
        }
    }
